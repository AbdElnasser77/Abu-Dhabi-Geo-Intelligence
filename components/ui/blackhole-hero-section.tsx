...
export function BlackHoleHeroSection({
  ...
  /**
   * The live prop values, read from inside the render loop.
   *
   * Upstream this was a ref reassigned during render. That pattern is a lint
   * error in this project — `react-hooks` forbids writing a ref while
   * rendering, and the React Compiler is free to skip the assignment
   * altogether. `useEffectEvent` is the sanctioned replacement and is already
   * how `components/map/map-canvas.tsx` reaches live props from a long-lived
   * effect: the effect below mounts once and never re-runs, and each animation
   * frame calls this to pick up whatever the props are at that moment.
   */
  const readConfig = useEffectEvent(() => ({
  ...
  useEffect(() => {
    ...
    const isGL2 =
      ...
      glRaw instanceof WebGL2RenderingContext;

    /*
     * WebGL 1 is the common denominator and every draw call below lives in it.
     * TypeScript models `WebGL2RenderingContext` as a sibling interface rather
     * than a subtype — their overload sets differ — so a shared method reached
     * through a union of the two is not callable at all. Narrowing to the 1.0
     * surface is what lets the rest of this file read normally; `gl2` is kept
     * only for the float-buffer probe, the one place the 2.0 API is needed.
     */
    const gl = glRaw as WebGLRenderingContext;
    const gl2 = isGL2 ? (glRaw as WebGL2RenderingContext) : null;
    ...
    function resize() {
      ...
      const dpr = software
        ? 1
        : Math.min(window.devicePixelRatio || 1, Math.max(1, C.maxDpr));
      const cssW = Math.max(1, Math.round(rect.width));
      const cssH = Math.max(1, Math.round(rect.height));
      const scale = software ? 0.34 : Math.min(1, Math.max(0.4, C.resolution));
    ...
    function render(t: number) {
      ...
      const halton = HALTON[settled % HALTON.length];
      gl.uniform2f(u.uJitter!, halton[0] - 0.5, halton[1] - 0.5);
    ...
    function tick(now: number) {
      ...
      lastFrame = now;
      if (!readConfig().paused && !reduced) clock += dt;
      render(clock);
    }
    ...
    const ro = new ResizeObserver(() => {
      resize();
      if (reduced || readConfig().paused) settle(16);
    });
    ro.observe(host);
